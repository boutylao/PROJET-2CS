...
import { RapportDetailDialog } from './RapportDetailDialog';
...
export function AnalyseTable({
  ...
  const [rapportData, setRapportData] = React.useState<any>(null);
  ...
  const handleOpenDetailDialog = async (reportIdWithPrefix: string) => {
    const reportId = reportIdWithPrefix.replace('REP-', '');
  
    try {
      const response = await fetch(`http://localhost:8099/api/reports/${reportId}`);
      if (!response.ok) {
        console.error("Erreur lors du fetch du rapport :", await response.text());
        return;
      }
      const data = await response.json();
      console.log('Rapport data:', data); // ← pour vérifier
      setRapportData(data);
      setSelectedReportId(reportIdWithPrefix); // on garde l'ID original pour suivi
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <IconButton color="info" onClick={() => handleOpenDetailDialog(row.id)}>
